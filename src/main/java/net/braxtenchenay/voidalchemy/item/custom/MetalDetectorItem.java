...
import net.braxtenchenay.voidalchemy.util.ModTags;
...
public class MetalDetectorItem extends Item {
    public MetalDetectorItem(Settings settings, Model model) {
        super(settings);
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(!context.getWorld().isClient()) {
            ...
            if(!foundBlock) {
                player.sendMessage(Text.translatable("item.voidalchemy.metal_detector.no_valuables"));
            }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.voidalchemy.metal_detector.tooltip.shift"));
        } else {
            tooltip.add(Text.translatable("tooltip.voidalchemy.metal_detector.tooltip"));
        }
    }
